...
- **gpt-image-2 优先，现有方式兜底**：封面和正文配图先尝试 `@gpt-image-2`；只有在依赖缺失、调用失败、超时、无图产出或人工检查明显不可用时，才降级到 HTML/Playwright 或 `@generate-wechat-theme` 的基础封面。
...
### 阶段 3.5：正文配图生成（优先 gpt-image-2，失败降级）

用户确认完稿后，对每个 `[IMG:type]` 占位符执行生图。默认顺序：

1. **首选：`@gpt-image-2`**
2. **降级：现有 HTML 生图 + Playwright 截图**
...
例外：如果用户明确要求 gpt-image-2 产图用于验收，不要用 HTML/Playwright 降级图替代验收稿；应说明失败原因并继续重试或让用户决定是否接受降级。

降级时只需在最终报告中说明一句：`正文图 X 已因 gpt-image-2 不可用/无有效产出，改用 HTML 截图生成。`
...
封面也采用同样的两级策略：先用 `@gpt-image-2` 生成 900×383 封面；失败时保留 `@generate-wechat-theme` publish 脚本内置封面生成。
...
当 gpt-image-2 不可用或封面无有效产出时，可调用 `@generate-wechat-theme` 的 publish 脚本生成基础草稿或借用其转换能力：

注意：如果用户明确要求 gpt-image-2 封面用于验收，本节降级路径不得用于创建验收草稿。
...
3. 若 gpt-image-2 封面成功，上传该封面并使用返回的 `thumb_media_id`；否则使用 publish 脚本生成的基础封面
...
| gpt-image-2 无图产出 | Codex CLI 未登录、图像权益不可用、网络/模型失败 | 记录原因，改用 HTML/Playwright 或内置封面 |
